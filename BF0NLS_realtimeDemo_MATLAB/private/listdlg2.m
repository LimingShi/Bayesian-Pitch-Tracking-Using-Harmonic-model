function [selection,value] = listdlg2(varargin)
%LISTDLG  List selection dialog box.
%   [SELECTION,OK] = LISTDLG2('ListString',S) creates a modal dialog box
...
% simple test:
%
% d = dir; [s,v] = listdlg2('PromptString','Select a file:','ListString',{d.name});
% 

% Generate a warning in -nodisplay and -noFigureWindows mode.
% warnfiguredialog('listdlg');
%  usedpath=strcat(matlabroot,'/toolbox/matlab/uitools/private');
%  addpath(genpath(usedpath));
narginchk(1,inf)
...
if isempty(pos)
   ...
   set(fig, 'Position', pos); % set the figure position as desired using custom code
end

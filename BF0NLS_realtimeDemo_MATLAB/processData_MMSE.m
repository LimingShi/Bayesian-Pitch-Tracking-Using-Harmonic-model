function Obj=processData_MMSE(Obj, event)
% tic
...
[frame_p,filter_state] = filter([mmse_lpc],1,...
    frame);
Obj.UserData{26}=filter_state;
else
    [frame_p,Obj.UserData{26}] = filter([mmse_lpc],1,...
    frame,Obj.UserData{26});
end
